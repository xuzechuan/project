...
const servers = https.createServer({
  key: fs.readFileSync('/root/.acme.sh/xuzechuan.com/xuzechuan.com.key'),
  cert: fs.readFileSync('/root/.acme.sh/xuzechuan.com/xuzechuan.com.cer'),
});
const cookieSecret = 'cookie sign secert';

// WebSocketServer接管了server的upgrade事件
const wss1 = new WebSocketServer({ server: server });
const wss2 = new WebSocketServer({ server: servers });
  ...
  if (vote) {
    ...
    if (multiple) {
      ...
      if (vote.anonymous) {
        ...
        // 先看用户是否已经投过这个选项
        let voted = db
          .prepare('SELECT * FROM voteOptions WHERE userId = ? AND voteId = ? AND optionId = ?')
        ...
        if (voted) {
          ...
          db.prepare('INSERT INTO voteOptions (userId, voteId, optionId) VALUES (?, ?, ?)').run(
            userId,
            voteId,
            optionId
          );
...
servers.listen(443, () => {
  console.log('vote app listening on port', 443);
});
server.listen(80, () => {
  console.log('vote app listening on port', 80);
});
